        ...
        try:
            ...
            response = self.client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    ...
                    temperature=1.2 if self.act_level == 7 else 0.7,
        ...
        try:
            ...
            response = model.generate_content(
                ...
                generation_config=genai.types.GenerationConfig(
                    max_output_tokens=max_tokens,
                    # Level 7 可以把温度调高，增加“混乱度”
                    temperature=1.2 if self.act_level == 7 else 0.7, 
                ),
                safety_settings=safety_settings # 注入动态设置
            )
            
            # 检查是否因为安全原因被拦截导致 response.text 为空
            if response.candidates[0].finish_reason == 3: # 3 代表 SAFETY
                answer = "⚠️ [Gemini 拒绝回答：触发了硬性安全底线]"
            else:
                answer = response.text
